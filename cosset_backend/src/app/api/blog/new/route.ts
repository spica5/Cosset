...
/**
 * Create a new blog
 * Body: { blog: { customerId, title, category, description, content, file, isPublic/public, totalViews, following, fontPreset, backgroundPreset, comments } }
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const createdBlog = await createBlog({
      ...
      fontPreset:
        typeof blog.fontPreset === 'string'
          ? blog.fontPreset.trim()
          : blog.fontPreset != null
            ? String(blog.fontPreset)
            : null,
      backgroundPreset:
        typeof blog.backgroundPreset === 'string'
          ? blog.backgroundPreset.trim()
          : blog.backgroundPreset != null
            ? String(blog.backgroundPreset)
            : null,
    ...
    return handleError('Blog - Create', error as Error);
  }
}
