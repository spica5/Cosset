...
import {
  createBookshelfEbookReadingComment,
  getBookshelfEbookReadingComments,
...
export async function GET(req: NextRequest) {
  try {
    ...
    const comments = await getBookshelfEbookReadingComments(bookId, customerId);

    return response({ comments }, STATUS.OK);
  } catch (error) {
    return handleError('Bookshelf E-book Reading Comments - List', error as Error);
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const created = await createBookshelfEbookReadingComment({
      bookId,
      customerId,
      comment,
    ...
    return handleError('Bookshelf E-book Reading Comments - Create', error as Error);
  }
}
