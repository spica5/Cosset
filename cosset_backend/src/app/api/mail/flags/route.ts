...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const flags = {
      ...
      isUnread: typeof body?.isUnread === 'boolean' ? body.isUnread : undefined,
    };

    if (
      flags.isStarred === undefined &&
      flags.isImportant === undefined &&
      flags.isUnread === undefined
    ) {
      return response({ message: 'No mail flags to update' }, STATUS.BAD_REQUEST);
    }

    const row = await updateUserMailFlags(userId, mailId, flags);
    if (!row) {
    ...
    return handleError('Mail - Update flags', error as Error);
  }
}
