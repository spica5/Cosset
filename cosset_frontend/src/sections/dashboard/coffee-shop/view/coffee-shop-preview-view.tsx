...
export function CoffeeShopPreviewView({ coffeeShopId }: Props) {
  ...
  if (!coffeeShop) {
    return (
      <Box sx={{ minHeight: '100vh', display: 'grid', placeItems: 'center', bgcolor: '#0b0f14', p: 3 }}>
  ...
  const hasImage = resolvedBackground.startsWith('http://') || resolvedBackground.startsWith('https://');
  ...
  return (
    ...
    >
      <Stack sx={{ p: 2 }}>
        <Typography variant="subtitle2" sx={{ color: 'common.white', textShadow: '0 2px 8px rgba(0,0,0,0.45)' }}>
