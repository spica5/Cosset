...
type Props = {
  /** collectionId: 1 = Good Memo, 2 = Sad Memo */
  collectionId: 1 | 2;
  currentMemo?: IMemoItem;
  onSaved?: () => void;
  onClose?: () => void;
};
...
export function MemoForm({ collectionId, currentMemo, onSaved, onClose }: Props) {
  const { user } = useAuthContext();

  const [form, setForm] = useState<MemoFormState>(emptyForm);
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      date: toDateInputValue(currentMemo.date),
      isPublic: currentMemo.isPublic === 1,
    });

    setImageKeys(parseStorageKeys(currentMemo.images));
  }, [currentMemo]);

  // Load signed preview URLs for existing image keys
  useEffect(() => {
    let mounted = true;
    ...
    try {
      ...
      const uploadedKeys = await Promise.all(
        pendingFiles.map(async (file) => {
          ...
          const key = `collection-items/${collectionId}/images/${uuidv4()}.${ext}`;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const payload = {
      collectionId,
      customerId: currentMemo?.customerId || (user?.id ? String(user.id) : undefined),
    ...
    try {
      ...
      if (currentMemo?.id) {
        await updateCollectionItem(currentMemo.id, payload);
  ...
  }, [collectionId, currentMemo, form, imageKeys, onSaved, user?.id]);
            ...
            <FormControlLabel
              ...
              control={
                <Switch
                  ...
                  inputProps={{ 'aria-label': 'memo-public-switch' }}
                />
              }
            />
