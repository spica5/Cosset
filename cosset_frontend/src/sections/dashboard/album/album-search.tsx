...
export function AlbumSearch({ search, onSearch }: Props) {
  ...
  const handleKeyUp = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (state.query) {
      if (event.key === 'Enter') {
        const selectProduct = state.results.filter((album) => album.title === state.query)[0];
  ...
  return (
    <Autocomplete
      ...
      renderOption={(props, album, { inputValue }) => {
        const matches = match(album.title, inputValue);
        const parts = parse(album.title, matches);

        return (
          <Box component="li" {...props} onClick={() => handleClick(album.id)} key={album.id}>
            <Avatar
              key={album.id}
              alt={album.title}
              src={album.coverUrl}
