...
import { NavLi, NavUl } from 'src/components/universe/nav-section';
...
function NavSubList({ subheader, coverUrl, items }: NavSubListProps) {
  const pathname = usePathname();
...
const useClientRect = (ref: React.RefObject<HTMLElement>): { top: number; height: number } => {
  const [clientRect, setClientRect] = useState<{ top: number; height: number }>({
    top: 0,
    height: 0,
  });
