...
type Props = {
  ...
  searchParams: Promise<{ ownerId?: string; filmId?: string }>;
};
...
export default async function Page({ params, searchParams }: Props) {
  ...
  const { ownerId, filmId } = await searchParams;

  if (!isCinemaCategory(category)) {
    notFound();
  }

  const parsedFilmId = filmId ? Number.parseInt(filmId, 10) : undefined;

  return (
    <UniverseCinemaView
      categoryId={category}
      ownerId={ownerId}
      initialFilmId={Number.isFinite(parsedFilmId) ? parsedFilmId : undefined}
    />
  );
}
