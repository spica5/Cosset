...
/**
 * Get S3 signed URL from storage key
 * @param key - S3 object key
 * @returns Signed S3 URL
 */
export async function getS3SignedUrl(key?: string | null): Promise<string> {
  if (!key) return '';
  
  try {
    const { default: axiosInstance, endpoints } = await import('./axios');
